...
export type CommonDonutDetailItem = {
  label: string
  value: number | string
  unit?: string
  color: string
...
export default function CommonDonutDetail({
  legend,
  summaryLabel = '요약',
}: CommonDonutDetailProps) {
  ...
  return (
    <div className={mmc.dashboard_ratioBody}>
      ...
      <CddSide>
        <ul className={mmc.dashboard_legend}>
          {prepared.map((item, idx) => (
            <li
              key={item.label}
